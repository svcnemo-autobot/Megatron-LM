...
def safe_extract_zip(archive: zipfile.ZipFile, destination: str | Path) -> None:
    ...
    archive.extractall(destination)  # nosec B202 - member paths were validated above
...
def safe_extract_tar(archive: tarfile.TarFile, destination: str | Path) -> None:
    ...
    archive.extractall(destination, members=members)  # nosec B202 - members were validated above
