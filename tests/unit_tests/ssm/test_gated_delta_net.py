...
class TestGatedDeltaNet:
    ...
    def test_gpu_forward_thd_correctness(self):
        ...
        hidden_states_sbhd = torch.empty(
...
class TestFusedThdAllToAll:
    ...
    def test_cp2hp_batched_matches_per_seq(self, cu_seqlens, split_sections):
        ...
        torch.manual_seed(42)
        local_t = (
            torch.empty(T_local, 1, hidden, device=torch.cuda.current_device()).uniform_()
            .bfloat16()
            .contiguous()
        )
    ...
    def test_hp2cp_batched_matches_per_seq(self, cu_seqlens):
        ...
        torch.manual_seed(42)
        global_t = (
            torch.empty(T_global, 1, h_local, device=torch.cuda.current_device()).uniform_()
            .bfloat16()
            .contiguous()
        )
