...
class TestReadGoldenValuesFromJson:
    ...
    def test_missing_file_raises_value_error_naming_path(self, tmp_path):
        """A missing golden-values file must raise the documented ``ValueError``.

        ``read_golden_values_from_json`` guards on ``os.path.exists`` and is
        written to ``raise ValueError(f"File {golden_values_path} not found!")``
        for a path that does not exist. That is the contract callers rely on to
        distinguish "golden values were never generated" from an arbitrary IO
        error. This test pins that contract: the raised exception must be a
        ``ValueError`` (not merely any ``OSError``) and must name the missing
        path.
        """
