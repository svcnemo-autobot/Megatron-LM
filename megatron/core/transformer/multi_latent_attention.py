...
class MultiLatentAttention(Attention):
    ...
    def _project_and_apply_mla_output_gate(
        ...
        """Project and apply the configured MLA output gate."""
    ...
    def _apply_mla_output_gate(
        ...
        """Apply an elementwise or headwise MLA output gate."""
        ...
        if self.config.gated_attention_proj_granularity == 'elementwise':
            if core_attn_out.shape != gate.shape:
                raise ValueError(
                    "Elementwise MLA output gating requires matching shapes, got "
                    f"{tuple(gate.shape)} and {tuple(core_attn_out.shape)}."
                )
            gate = torch.sigmoid(gate.float()).to(core_attn_out.dtype)
            return core_attn_out * gate
        if core_attn_out.size(-1) % gate.size(-1) != 0:
            raise ValueError(
                "Headwise MLA output gating requires an output dimension divisible by "
                f"the number of local gates, got {core_attn_out.size(-1)} and {gate.size(-1)}."
            )
    ...
    def _apply_mla_headwise_output_gate(
        ...
        """Apply one gate per local MLA attention head."""
        output_shape = core_attn_out.shape
        core_attn_out = core_attn_out.view(*output_shape[:2], gate.size(-1), -1)
        gate = torch.sigmoid(gate.float()).to(core_attn_out.dtype)
        return (core_attn_out * gate.unsqueeze(-1)).reshape(output_shape)
