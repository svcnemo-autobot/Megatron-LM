...
def _indexer_topk_single_packed_cp_segments(
    ...
    b, sq, _idx_nh, _idx_hd = q_bshd.shape
    sk = k_bshd.size(1)
    if b != 1 or sq % 2 != 0 or _INDEXER_RATIO != 1:
        raise RuntimeError("single packed CP cuDNN fast path requires b=1, even sq, ratio=1")

    half = sq // 2
    segment_specs = (
        (0, half, (local_packed_cp_rank + 1) * half),
        (half, sq, sk - local_packed_cp_rank * half),
    )
    indices_chunks = []
    values_chunks = [] if return_topk_scores else None

    for row_start, row_end, key_end in segment_specs:
        ...
        segment_len = row_end - row_start
        segment_start = key_end - segment_len
        seq_lens = torch.arange(
            segment_start + 1, key_end + 1, dtype=torch.int32, device=q_bshd.device
        )
    ...
    if starts is None:
        ...
        if not return_scores and use_local_indexer_varlen and single_packed_thd_sequence:
            topk_indices, topk_scores = _indexer_topk_single_packed_cp_segments(
                q_bshd, k_bshd, w_bsh, topk_k, return_topk_scores, local_packed_cp_rank
            )
...
class FusedQKTopKWithSparseLossFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_topk_indices, grad_topk_length, grad_loss):
        ...
        #   single_packed_thd_sequence, local_packed_cp_rank, tp_group.
        return (
            grad_q_indexer,
            grad_k_indexer,
            grad_weights,
